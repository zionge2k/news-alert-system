...
class YTNCrawler(BaseCrawler):
    ...
    def __init__(self, base_url: str = "https://www.ytn.co.kr"):
        """
        YTN 크롤러 초기화

        Args:
            base_url (str): YTN 뉴스 기본 URL
        """
        self.base_url = base_url

    async def fetch(self) -> str:
        """
        YTN 뉴스 사이트에서 최신 뉴스 HTML을 가져옴

        Returns:
            str: 수집된 HTML 콘텐츠
        """
        url = f"{self.base_url}/news/main.html"
        headers = {
        ...
        try:
            ...
            logger.error(f"YTN 뉴스 가져오기 실패: {e}")
            raise
    ...
    def parse(self, html: str) -> Dict[str, Any]:
        ...
        news_item = soup.select_one("div.newslist_area li")
        if not news_item:
            logger.warning("YTN 뉴스 항목을 찾을 수 없음")
            return {}

        title_element = news_item.select_one("strong.tit")
        content_element = news_item.select_one("p.desc")
