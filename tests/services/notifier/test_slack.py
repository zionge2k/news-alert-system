...
class TestSlackNotifier:
    ...
    @pytest.fixture
    def notifier(self):
        """테스트용 SlackNotifier 인스턴스 생성"""
        return SlackNotifier(webhook_url="https://hooks.slack.com/services/test")
    ...
    def test_format_message(self, notifier, mock_news):
        ...
        # 필수 요소가 포함되어 있는지 확인
        assert "*📰" in message  # 이모지와 마크다운 볼드 포함
        ...
        # Slack 특화 마크다운 문법 확인
        assert "<" + mock_news["url"] + "|뉴스 원문 보기>" in message
    ...
    async def test_send_success(self, notifier):
        ...
        mock_response.status = 200  # Slack API 성공 응답
